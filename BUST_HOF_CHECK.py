...
def last_run_date(log_file):
    if os.path.exists(log_file):
        with open(log_file, "r") as file:
            ...
            if len(reader) >= 1 and reader[0]!=[]:  # Check if there's at least one data row
...
def plot_progress(log_file, goal_ranking, webhook_url, discordID, cat):
    ...
    plt.plot(dates, current_ranks, label=f"Current Rank in {cat}", color="blue")
    plt.axhline(goal_ranking, color="red", linestyle="--", label=f"Goal Rank: {goal_ranking}")
    
    plt.xlabel("Date")
    plt.ylabel("Rank")
    plt.title(f"Progress to Goal Rank {goal_ranking} in {cat}")
    plt.legend()
...
def process_user_data():
    ...
    with open(r'C:\Users\vedic\Desktop\pt\Torn-Hall-Of-Fame-Tracker\checkers.json', 'r') as f:
        users = json.load(f)
